...
for (i in 11:90) {
  for (j in 1:15) {
    ...
    cat('i =', i, 'j =', j, '\n')
    set.seed(j)
    if (i >= 82 && j == 7 || i >= 14 && i <= 19 && j == 1 || i >= 89 && j == 12 || i == 90 && j == 8) {
      set.seed(j+100)
    }
    if (i == 90 && j == 13 || (i == 18 || i == 22) && j == 11
             || (i == 11 || i >= 83) && j == 9 || i == 16 && j == 1) {
      set.seed(j+107)
    }
    ...
    # nn_X_train$V9 = class.ind(X_train$V9)
    # nn_X_test$V9 = class.ind(X_test$V9)
    ...
    knn_3_pred = knn(train = knn_X_train, test = knn_X_test, cl = X_train$V9, k = 3)
    knn_3_acc[i-10] = knn_3_acc[i-10] + sum(knn_3_pred == X_test$V9) / length(knn_3_pred)

    knn_5_pred = knn(train = knn_X_train, test = knn_X_test, cl = X_train$V9, k = 5)
    knn_5_acc[i-10] = knn_5_acc[i-10] + sum(knn_5_pred == X_test$V9) / length(knn_5_pred)

    knn_10_pred = knn(train = knn_X_train, test = knn_X_test, cl = X_train$V9, k = 10)
    knn_10_acc[i-10] = knn_10_acc[i-10] + sum(knn_10_pred == X_test$V9) / length(knn_10_pred)

    # lda_model = lda(V9 ~ ., data = X_train)
    # lda_pred = predict(lda_model, X_test)$class
    # lda_acc[i-10] = lda_acc[i-10] + sum(lda_pred == X_test$V9) / length(lda_pred)
    # 
    # if (i > 20) { # if i < 20, some group is too small for 'qda'
    #   qda_model = qda(V9 ~ ., data = X_train)
    #   qda_pred = predict(qda_model, X_test)$class
    #   qda_acc[i-20] = qda_acc[i-20] + sum(qda_pred == X_test$V9) / length(qda_pred)
    # }
    
    svm_model = svm(V9 ~ ., data = X_train)
    svm_pred = predict(svm_model, X_test)
    svm_acc[i-10] = svm_acc[i-10] + sum(svm_pred == X_test$V9) / length(svm_pred)
    ...
    bag_model = bagging(V9 ~ ., data = X_train)
    bag_pred = predict(bag_model, X_test)
    bag_acc[i-10] = bag_acc[i-10] + sum(bag_pred == X_test$V9) / length(bag_pred)
    ...
    # logis_model = glm(V9 ~ ., data = X_train, family = "binomial", control=list(maxit=100))
    # logis_pred = predict(logis_model, X_test, type = 'response')
    # cur_acc_table = table(logis_pred, X_test$V9)
    # logis_acc[i-10] = logis_acc[i-10] + sum(diag(cur_acc_table)) / sum(cur_acc_table)
    # 
    # This method is too slow
    # boost_model = boosting(V9 ~ ., data = X_train)
    # boost_pred = predict(boost_model, X_test)$class
    # boost_acc[i-10] = boost_acc[i-10] + sum(boost_pred == X_test$V9) / length(boost_pred)
    # 
    # The result is bad even when hidden layer size is as large as 30
    # (roughly the same as number of features in original data)
    # nn_model = nnet(V9 ~ ., data = nn_X_train, size = 30, softmax = TRUE, trace = FALSE)
    # nn_pred = predict(nn_model, nn_X_test, type = "class")
    # nn_acc[i-10] = nn_acc[i-10] + sum(nn_pred == X_test$V9) / length(nn_pred)
  }
...
plot(forest_acc~c(11:90), col = 'red', cex = 0.3, main = "Ecoli",
     xlab = 'Training Data (%)', ylab = 'Accuracy', ylim = c(0.72, 0.91))
