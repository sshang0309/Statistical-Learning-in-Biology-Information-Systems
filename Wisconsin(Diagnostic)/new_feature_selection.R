...
for (i in 11:90) {
  for (j in 1:15) {
    ...
    sbc_rf_model = naiveBayes(V2 ~ V25 + V26 + V23 + V30 + V10, data = X_train)
    ...
    sbc_model = naiveBayes(V2 ~ V25 + V7 + V30 + V9 + V12 + V23 + V21, data = X_train)
...
plot(sbc_acc~c(11:90), col = 'red', cex = 0.3, main = "Breast Cancer Wisconsin (Diagnostic)",
     xlab = 'Training Data (%)', ylab = 'Accuracy', ylim = c(0.9, 0.97))
