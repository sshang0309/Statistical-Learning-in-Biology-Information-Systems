...
select_var = cancer[, (names(cancer) %in% c('V35', 'V34', 'V30', 'V10', 'V18'))]
...
for (i in 11:90) {
  for (j in 1:15) {
    ...
    ssvm_model = svm(V2 ~ V35 + V34 + V30 + V10 + V18, data = X_train)
    ssvm_pred = predict(ssvm_model, X_test)
    ssvm_acc[i-10] = ssvm_acc[i-10] + sum(ssvm_pred == X_test$V2) / length(ssvm_pred)
    
    # slda_model = lda(V2 ~ spread1 + D2 + spread2 + MDVP.Flo.Hz. + MDVP.Fo.Hz. + NHR, data = X_train)
    ...
    #   sqda_model = qda(V2 ~ spread1 + D2 + spread2 + MDVP.Flo.Hz. + MDVP.Fo.Hz. + NHR, data = X_train)
...
plot(knn_3_acc~c(11:90), col = 'red', cex = 0.3, main = "Breast Cancer Wisconsin (Prognostic)",
     xlab = 'Training Data (%)', ylab = 'Accuracy', ylim = c(0.66, 0.81))
...
plot(svm_acc~c(11:90), col = 'red', cex = 0.3, main = "Breast Cancer Wisconsin (Prognostic)",
     xlab = 'Training Data (%)', ylab = 'Accuracy', ylim = c(0.74, 0.81))
...
plot(svm_acc~c(11:90), col = 'red', cex = 0.3, main = "Breast Cancer Wisconsin (Prognostic)",
     xlab = 'Training Data (%)', ylab = 'Accuracy', ylim = c(0.62, 0.81))
points(c(11:90), tree_acc, col = 'blue', cex = 0.3)
points(c(11:90), sbc_acc, col = 'darkgreen', cex = 0.3)
points(c(11:90), knn_10_acc, col = 'purple', cex = 0.3)
lines(svm_acc~c(11:90), col = 'red')
lines(tree_acc~c(11:90), col = 'blue')
lines(sbc_acc~c(11:90), col = 'darkgreen')
lines(knn_10_acc~c(11:90), col = 'purple')
legend("bottomright",
       c("SVM", "C4.5", "SBC", "KNN(k=10)"),
